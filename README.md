# the-jopetnah-foundation
The Jopetnah Foundation: integrated education, health, livelihoods &amp; environment programs for underserved communities. Founding stage — legal registration in progress.
<div align="center">

# 🌱 The Jopetnah Foundation

**Building whole communities, one life at a time.**

*Education · Health · Livelihoods · Environment*

[![Status](https://img.shields.io/badge/status-founding%20stage-orange)]()
[![Stage](https://img.shields.io/badge/stage-seeking%20founding%20partners-blueviolet)]()
[![License](https://img.shields.io/badge/license-CC%20BY--NC%204.0-lightgrey)]()

[Mission](#-mission) · [Programs](#-programs) · [Funding Strategy](#-funding-strategy) · [Grants Calendar](#-grants-calendar) · [Flagship Events](#-flagship-fundraising-events) · [Website Copy](#-website-copy--diaspora-giving-circle) · [Governance](#-governance--accountability) · [Roadmap](#-launch-roadmap) · [Get Involved](#-get-involved)

</div>

---

> This repository is the working home for The Jopetnah Foundation's mission, programs, and fundraising plans as we finalize registration with legal counsel. Details will be updated as that process completes.

## 🎯 Mission

To advance **education, health, economic opportunity, and environmental resilience** in underserved communities through integrated, locally-led programs.

**Vision:** A generation of communities that are educated, healthy, economically self-reliant, and environmentally sustainable.

**Values:** Integrity · Community ownership · Transparency · Sustainability · Inclusion

**Why one foundation, four pillars?** Funders increasingly favor organizations that show how issues interconnect — a sick child can't stay in school; a family without income can't afford healthcare. Jopetnah's positioning as an *integrated development* foundation, rather than four disconnected charities, is itself a fundraising advantage: it lets us pitch a whole-family, whole-community model to donors who want higher-impact-per-dollar stories.

## 💡 The Problem

In the communities Jopetnah serves, a child out of school, a family without healthcare, and a household without income are rarely separate problems — they're the same problem, seen from different angles. Most charities specialize in one slice of it. **Jopetnah is built to address the whole picture, for the whole family.**

```
            ┌───────────────────────────┐
            │   ONE FAMILY, ONE GIFT    │
            └─────────────┬─────────────┘
     ┌───────────┬─────────┴─────────┬───────────┐
     ▼           ▼                   ▼           ▼
 Education     Health           Livelihoods   Environment
```

## 🏛️ Organizational Structure

| Layer | Role |
|---|---|
| **Board of Trustees** | 7–9 members: governance, fiduciary oversight, high-level fundraising relationships |
| **Executive Director** | Day-to-day leadership, funder relationships, strategy |
| **Program Directors (x4)** | One per pillar: Education, Health, Livelihoods, Environment |
| **Fundraising & Partnerships Lead** | Owns donor pipeline, grants, corporate CSR deals |
| **Communications/Marketing Lead** | Storytelling, digital presence, campaign execution |
| **Advisory Council** | Respected community, diaspora, and sector figures |
| **M&E Officer** | Tracks and reports impact data for grant renewals and donor retention |

**Legal recommendation:** Register as a nonprofit/NGO with tax-exempt status where available — often a prerequisite for both corporate CSR partnerships and institutional grants.

## 🏗️ Programs

### 🎓 Education & Youth Development
- **Jopetnah Scholars** — need-based scholarships + mentorship for secondary/tertiary students
- **Future-Ready Labs** — digital literacy and vocational skills hubs in underserved schools
- **Read to Lead** — community libraries and literacy campaigns for early learners

### 🩺 Health & Wellbeing
- **Community Health Champions** — trains local volunteers as first-line health educators
- **Mobile Wellness Clinics** — periodic outreach to remote areas (maternal health, screenings, vaccinations)
- **Mind & Body Program** — mental health awareness, nutrition, and adolescent health education

### 💰 Poverty Alleviation & Livelihoods
- **Jopetnah Micro-Grants** — seed capital + training for small entrepreneurs (esp. women, youth)
- **Skills-to-Income Pipeline** — vocational training tied directly to job placement or self-employment
- **Cooperative Savings Circles** — savings groups paired with financial literacy training

### 🌱 Environment & Community Development
- **Green Neighborhoods** — tree planting, clean-up drives, community gardens
- **Water & Sanitation Access** — boreholes, filtration units, hygiene education
- **Climate-Smart Livelihoods** — links environment back to livelihoods (e.g., eco-friendly agribusiness)

*Cross-pillar design lets us package "bundled" giving — e.g., a $500 gift funds one family's full-circle support: a child's school fees, a health check-up, and a livelihood grant for a parent.*

## 💰 Funding Strategy

Three tracks running in parallel:

### Track A: Individual Donors & Diaspora
1. **"Send Home Hope" Diaspora Giving Circle** — monthly recurring giving targeted at diaspora communities abroad
2. **Founder's Circle** — recognition tier for donors giving above a threshold annually
3. **Birthday/Holiday Fundraisers** — peer-to-peer fundraising via social platforms around "Jopetnah Day"
4. **Sponsor-a-Student / Sponsor-a-Family** — recurring micro-donations tied to a specific beneficiary story

### Track B: Corporate Sponsors / CSR
1. **Tiered Corporate Partnership Program** (Bronze/Silver/Gold/Platinum)
2. **"Adopt-a-Program" Sponsorships** — a company sponsors one flagship program
3. **Employee Giving & Matching Campaigns**
4. **Cause-Marketing Product Tie-ins**

### Track C: Institutional Grants
1. Maintain a **grants calendar** (see below) tracking bilateral, multilateral, and private foundation calls
2. Develop **modular concept notes** per pillar for fast response to open calls
3. Pursue **government/consortium partnerships** as the fastest entry point to first awards
4. Invest early in **M&E systems** — grantmakers require rigorous impact data

**Giving tiers (draft, pending final currency/amounts with legal & finance):**

| Tier | Gift | Funds |
|---|---|---|
| Friend | $25/mo | One month of a community library slot |
| Champion | $100/mo | One month of a Community Health Champion's outreach |
| Guardian | $500 (one-time or /mo) | A full "whole-family" bundled gift |
| Founder's Circle | $5,000+/yr | Named recognition, gala invite, quarterly briefings |

## 📅 Grants Calendar

Real, current funding opportunities relevant to our pillars in Uganda/East Africa. *(Also maintained as an interactive spreadsheet with status-tracking dropdowns, shared alongside this repo.)*

| Funder / Program | Pillar(s) | Grant Type | Typical Range (USD) | Deadline / Cycle | Notes |
|---|---|---|---|---|---|
| Mastercard Foundation EdTech Fellowship (via Hive Colab) | Education | Catalytic / equity-free funding | Up to $70,000 | Annual — 2026 cycle closed 5-Feb-2026; watch for 2027 call | Best fit if Jopetnah spins up a dedicated EdTech/digital-learning venture arm |
| Mastercard Foundation Scholars Program – Makerere University | Education | Scholarship placement | Full tuition + stipend | Annual — 2026/27 deadline 5-Jun-2026 | Route candidates from Jopetnah Scholars here |
| Segal Family Foundation | Education / Health / Livelihoods / Environment | Unrestricted, multi-year grant | $5,000–$250,000 | Rolling, referral-based | Apply directly + seek a warm referral through an existing partner |
| GEF Small Grants Programme (UNDP Uganda) | Environment / Livelihoods | Project grant | Up to $40,000–$50,000 | Rolling calls (recent call closed 2-Jul-2026) | Strong fit for Green Neighborhoods / Water & Sanitation |
| MTN Uganda Foundation — Changemakers Initiative | Education / Health / Environment / Livelihoods | Community grant | Share of ~$130,000 pool / ~25 projects | Phased calls; Phase 3 launched Oct 2025 | Requires co-branding with MTN |
| MTN Uganda Foundation — General Partnership Proposals | Education / Health / ICT / Youth | Sponsorship / CSI grant | Varies | Rolling, reviewed quarterly | Separate from Changemakers |
| USAID Uganda Mission — open solicitations | All pillars | Institutional grant / cooperative agreement | Often $100,000+ | Continuous — check grants.gov / SAM.gov | Best pursued via consortium with an established NGO first |
| Comic Relief — International Grants | Health / Education / Gender | Project grant | £30,000–£500,000+ | Check comicrelief.com | Usually needs a UK fiscal partner |
| Stanbic Bank Uganda CSR | Education / Livelihoods | Sponsorship / CSR partnership | Varies | Check annually, typically Q1 | Aligns with Skills-to-Income Pipeline |
| Airtel Uganda / Airtel Foundation CSR | Health / Education / ICT | Sponsorship / in-kind + cash | Varies | Rolling | Consider pitching Digital Literacy Labs |
| *Add your own opportunity* | | | | | |

**How to use this calendar:** update status (Not started → Researching → In progress → Submitted → Awarded/Declined) as you progress, assign an owner per opportunity, and re-verify deadlines quarterly — this list reflects information available as of **July 2026**. Treat rolling/no-deadline funders as always-open.

## 🎉 Flagship Fundraising Events

### Jopetnah Day (Annual 48-Hour Digital Giving Blitz)
Built for reach, not venue cost — ideal for diaspora and individual donors.
- **Goal:** 200+ new monthly recurring donors in Year 1
- **8-week runway:** confirm a matching-gift sponsor → recruit 15–20 Ambassadors → produce a #OneJopetnah story series → build a landing page with a live counter → tease, launch, and push for 48 hours → thank every donor within 48 hours

### The Jopetnah Gala (Annual Flagship Dinner)
The primary vehicle for major-donor cultivation and corporate sponsorship renewal.
- **Goal:** secure/renew 3–5 anchor corporate sponsors, convert mid-level donors into Founder's Circle members
- **16-week runway:** set date/venue → finalize sponsorship tiers → invite honorees → open ticket sales → send formal invitations → rehearse → hold the event → send personalized thank-yous within 5 business days
- **Typical run of show:** reception → welcome remarks → mission moment (beneficiary story) → dinner → honoree recognition → **the ask** (fund-a-need pledge moment) → live auction → closing remarks

**Sponsorship tiers:** Presenting Sponsor · Gold · Silver · Community Table (discounted, for grassroots partners/beneficiary families)

## 🌐 Website Copy & Diaspora Giving Circle

### Homepage Hero
**Headline:** Building whole communities, one life at a time.
**Subhead:** The Jopetnah Foundation brings education, health, economic opportunity, and environmental care together — because families don't face one problem at a time, and neither should we.

### Trust Strip *(place near the donate button)*
- ✅ Governed by an independent Board of Trustees
- ✅ Annual audited financials published in full
- ✅ Locally-led programs, decisions made by the communities we serve
- ✅ [X]% of every dollar goes directly to programs *(insert real figure once available)*

### "Send Home Hope" Diaspora Giving Circle
**Send Home Hope.** *A giving circle for the Jopetnah diaspora community — because home is never as far as it feels.*

You left, but your community didn't leave your heart. Send Home Hope turns that pull into something concrete: a child staying in school, a mother getting a check-up, a neighbor starting a small business.

| Tier | Monthly Gift | What it funds |
|---|---|---|
| Seedling | $15 | One month of school supplies for one child |
| Rooted | $40 | One month of a Community Health Champion's outreach |
| Grove | $100 | One family's monthly "whole-family" bundled support |
| Elder | $250+ | Recognition in our diaspora newsletter + invite to virtual town halls |

**Why recurring, not one-time?** A single $500 gift helps one family for one moment. A $40/month gift helps one family for a whole year — and lets Jopetnah plan programs with confidence instead of guessing what next quarter's funding will look like.

## 🏛️ Governance & Accountability

*(Reflects current planning intent — final legal form to be confirmed with counsel.)*

- Independent **Board of Trustees** (7–9 members) for fiduciary oversight
- **Executive Director** + Program Directors for each of the four pillars
- **Advisory Council** of community, diaspora, and sector leaders
- Commitment to published annual impact reports and audited financials once operational

## 🗺️ Launch Roadmap

- [ ] **Phase 1:** Legal registration & nonprofit status, board formation, brand + website launch, one pilot program per pillar
- [ ] **Phase 2:** First Jopetnah Day, 2–3 anchor corporate sponsors signed, first grant applications submitted
- [ ] **Phase 3:** Scale successful pilots, first Gala, multi-year institutional grants, geographic expansion

## 📂 Repository Structure

```
.
├── README.md          ← you are here (all program, funding, and event detail lives here)
├── index.html         ← public landing page (GitHub Pages)
├── LICENSE            ← CC BY-NC 4.0
└── CONTRIBUTING.md    ← how to get involved
```

**To publish the landing page:** enable **GitHub Pages** in the repo settings (Settings → Pages → Deploy from branch → `main` → `/root`), and `index.html` will be live at `https://<your-username>.github.io/<repo-name>/`.

*(A companion `Jopetnah_Grants_Calendar.xlsx` and `Jopetnah_Donor_Pitch_Deck.pptx` are maintained alongside this repo — not rendered in markdown, but referenced throughout.)*

## 🙋 Get Involved

We're currently formalizing Jopetnah's legal structure and looking for:
- Founding board members and advisors
- An initial legal/incorporation partner
- Early corporate and institutional co-founders

**Interested?** Open an issue in this repo, or reach out at `partnerships@jopetnahfoundation.org` *(placeholder — update once the domain/email is live)*.

## License

Content in this repository is shared under [CC BY-NC 4.0](LICENSE) — see [`LICENSE`](LICENSE) for details.

---

<div align="center">
<sub>© 2026 The Jopetnah Foundation</sub>
</div>
